...
function addNewHobby() {
  ...
  // Event listener for "Analyze My Resume" button
document.getElementById("analyzeButton").addEventListener("click", function() {
  analyzeSkills(); // Call the skill analysis function
});
}
  ...
  // Function to analyze skills and suggest jobs
// Function to analyze skills and suggest jobs
// Function to analyze skills and suggest jobs
// Function to analyze skills and suggest jobs
// Function to analyze skills and suggest jobs + additional skills
function analyzeSkills() {
  const skillsInput = document.getElementById('language').value.toLowerCase().trim();
  const toolsInput = document.getElementById('tool').value.toLowerCase().trim();
  const softwareInput = document.getElementById('softwares').value.toLowerCase().trim();

  // Check if skills are empty
  if (!skillsInput && !toolsInput && !softwareInput) {
      showPopup("⚠️ Please enter at least one skill.");
      return;
  }

  const allSkills = [...skillsInput.split(','), ...toolsInput.split(','), ...softwareInput.split(',')].map(skill => skill.trim());

  // Job roles mapped to required skills
  const jobRoles = {
      "Java Developer": ["java", "springboot", "sql"],
      ...
      "Data Scientist": ["python", "tensorflow", "numpy", "pandas"],
      "Database Administrator": ["sql", "mysql", "oracle"],
      "UI/UX Designer": ["figma", "adobe xd", "css", "html"]
  };

  let matchedJobs = [];
  let suggestedSkills = new Set();

  // Check which job roles match the user's skills
  for (let job in jobRoles) {
      let requiredSkills = jobRoles[job];
      let matchCount = requiredSkills.filter(skill => allSkills.includes(skill)).length;

      if (matchCount > 0) {
          matchedJobs.push(`${job} (Match: ${matchCount}/${requiredSkills.length})`);

          // Suggest additional skills that are missing for better job matching
  ...
  // Format the output
  let jobMessage = matchedJobs.length > 0
      ? `<strong>✅ Recommended Jobs:</strong><br> ${matchedJobs.join('<br>')}`
      : "⚠️ No matching jobs found. Try adding more relevant skills.";

  let skillMessage = suggestedSkills.size > 0
      ? `<br><strong>💡 Suggested Skills to Learn:</strong><br> ${[...suggestedSkills].join(', ')}`
      : "";

  // Show the results in a popup
  showPopup(jobMessage + skillMessage);
}
...
function showPopup(message) {
  ...
  popup.innerHTML = `
      <div style="
          ...
          max-width: 400px;
      ">
          <h2 style="color: #2177da;">🔍 Analysis Report</h2>
